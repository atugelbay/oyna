...
bool USerialCom::OpenWFC(int32 nPort, int32 nBaud, bool bDTR, bool bRTS)
{
	...
	dcb.BaudRate = nBaud;
	dcb.ByteSize = 8;
	// Explicitly pin 8-N-1 + no flow control. GetCommState inherits whatever
	// the Windows COM driver currently holds — e.g. parity=Even left over from
	// other software — which puts the controller into OLD bitmap scan mode
	// instead of NEW events. Verified against 12 USBPcap captures: Chinese
	// software uses SetData wValue=0x0008 (8-N-1), UE was 0x0208 (8-E-1).
	dcb.Parity       = NOPARITY;
	dcb.StopBits     = ONESTOPBIT;
	dcb.fParity      = 0;  // TRUE/FALSE hidden by HideWindowsPlatformTypes.h above
	dcb.fBinary      = 1;
	dcb.fOutxCtsFlow = 0;
	dcb.fOutxDsrFlow = 0;
	if (bDTR == true)
